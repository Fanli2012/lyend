...
class ArticleTypeModel extends BaseModel
{
    ...
    {
        return $this->hasMany(ArticleModel::class, 'type_id', 'id');
    }
    ...
    protected $common_field = array(
        'id', 'parent_id', 'name', 'seotitle', 'keywords', 'description', 'filename', 'templist', 'temparticle', 'litpic', 'listorder', 'add_time'
    );
    ...
    public function list_to_tree($parent_id = 0, $deep = 0)
    {
        ...
        $cats = $this->getDb()->where(['parent_id' => $parent_id])->orderBy('listorder', 'asc')->get();
