...
class UserLogic extends BaseLogic
{
    ...
    public function login($data)
    {
        ...
        $messages = [
            ...
            'password.min' => '密码6-18位',
            'password.max' => '密码6-18位',
        ];
        ...
        $map1 = [
            ['user_name', '=', $user_name],
            ['password', '=', $password],
            ['delete_time', '=', UserModel::USER_UNDELETE],
        ];
        $map2 = [
            ['email', '=', $user_name],
            ['password', '=', $password],
        ...
        $user = $this->getModel()->where($map1)->orWhere($map2)->orWhere($map3)->first();
        if (!$user) {
        ...
        if (isset($data['from']) && $data['from'] != '') {
            //生成Token
            $token = logic('Token')->getToken($user->id, $data['from']);
            if (!$token) {
    ...
    public function wxLogin($data)
    {
        ...
        //生成Token
        $token = logic('Token')->getToken($user->id, TokenModel::TOKEN_TYPE_WEIXIN);
        $user_info->token = $token;
    ...
    public function miniprogramWxlogin($data)
    {
        ...
        //生成Token
        $token = logic('Token')->getToken($user->id, TokenModel::TOKEN_TYPE_MINIPROGRAM);
        $user_info->token = $token;
    ...
    public function register($data)
    {
        ...
        $validator = $this->getValidate($data, 'register');
        if ($validator->fails()) {
            return ReturnData::create(ReturnData::PARAMS_ERROR, null, $validator->errors()->first());
        }
    ...
    public function userInfoUpdate($data, $where = array())
    {
        ...
        if (isset($data['user_name']) && !empty($data['user_name'])) {
            ...
            if ($this->getModel()->getOne($where2)) {
                return ReturnData::create(ReturnData::PARAMS_ERROR, null, '该用户名已存在');
            }
        }
        ...
        if (isset($data['email']) && !empty($data['email'])) {
            ...
            if ($this->getModel()->getOne($where2)) {
                return ReturnData::create(ReturnData::PARAMS_ERROR, null, '该邮箱已存在');
            }
        }
    ...
    public function userPasswordUpdate($data, $where = array())
    {
        ...
        $validator = $this->getValidate($data, 'user_password_update');
        if ($validator->fails()) {
            return ReturnData::create(ReturnData::PARAMS_ERROR, null, $validator->errors()->first());
        }
    ...
    public function userPayPasswordUpdate($data, $where = array())
    {
        ...
        $validator = $this->getValidate($data, 'user_pay_password_update');
        if ($validator->fails()) {
            return ReturnData::create(ReturnData::PARAMS_ERROR, null, $validator->errors()->first());
        }
    ...
    {
        $where['status'] = UserModel::USER_STATUS_NORMAL;
        $user = $this->getModel()->getOne($where);
        if (!$user) {
            return ReturnData::create(ReturnData::PARAMS_ERROR, null, '用户不存在');
        }
    ...
    {
        include_once base_path('app/Common/Librarys/Wechat/aes/wxBizDataCrypt.php');
        /**
    ...
    public function changeMobile($data, $where = array())
    {
        ...
        if (isset($data['mobile']) && !empty($data['mobile'])) {
            ...
            if ($this->getModel()->getOne($where2)) {
                return ReturnData::create(ReturnData::PARAMS_ERROR, null, '该手机号已存在');
            }
        }
    ...
    public function pcRegister($data)
    {
        ...
        $validator = $this->getValidate($data, 'pc_mobile_reg');
        if ($validator->fails()) {
            return ReturnData::create(ReturnData::PARAMS_ERROR, null, $validator->errors()->first());
        }

        //短信验证码校验
        $verify_code = model('VerifyCode')->isVerify(['mobile' => $data['mobile'], 'type' => $data['smstype'], 'code' => $data['smscode']]);
        if (!$verify_code) {
            return ReturnData::create(ReturnData::PARAMS_ERROR, null, '短信验证码不正确或已过期');
        }
    ...
    public function pcResetPwd($data)
    {
        ...
        $validator = $this->getValidate($data, 'pc_resetpwd');
        if ($validator->fails()) {
            return ReturnData::create(ReturnData::PARAMS_ERROR, null, $validator->errors()->first());
        }
