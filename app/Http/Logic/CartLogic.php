...
class CartLogic extends BaseLogic
{
    ...
    //购物车结算商品列表
    public function cart_checkout_goods_list($where)
    {
        $cart_ids = explode("_", $where['ids']);

        // 获取购物车列表
        $cartList = Cart::where(array('user_id' => $where['user_id']))->whereIn('id', $cart_ids)->get();
        $total_price = 0; //商品总金额
        $total_goods = 0; //商品总数量
        ...
        if ($cartList) {
            ...
            foreach ($cartList as $k => $v) {
                ...
                if (model('Goods')->bargain_price($goods->price, $goods->promote_start_date, $goods->promote_end_date) > 0) {
                    $cartList[$k]->is_promote = 1;
                }
