...
class IndexController extends BaseController
{
    ...
    public function category($cat, $page = 0)
    {
        ...
        if (cache("catid$cat")) {
            ...
            $post = object_to_array(DB::table('arctype')->where('id', $cat)->first());
            if (empty($post)) {
                return redirect()->route('page404');
            }
    ...
    public function detail($id)
    {
        ...
        if (cache("detailid$id")) {
            ...
            $post = object_to_array(DB::table('article')->where('id', $id)->first());
            if (empty($post)) {
                return redirect()->route('page404');
            }
    ...
    public function tag($tag, $page = 0)
    {
        ...
        $post = object_to_array(DB::table('tagindex')->where('id', $tag)->first());
        $data['post'] = $post;
    ...
    public function page($id)
    {
        ...
        if (!empty($id) && preg_match('/[a-z0-9]+/', $id)) {
            ...
            if (cache("pageid$id")) {
                ...
                $post = object_to_array(DB::table('page')->where($map)->first());
    ...
    public function productcat($cat, $page = 0)
    {
        ...
        $post = object_to_array(DB::table('product_type')->where('id', $cat)->first());
        if (empty($post)) {
            return redirect()->route('page404');
        }
        $data['post'] = $post;
    ...
    public function product($id)
    {
        ...
        $post = object_to_array(DB::table('product')->where('id', $id)->first());
        if (empty($post)) {
            return redirect()->route('page404');
        }
