...
<div class="zinfo" style="padding-left:20px;padding-right:20px;"><h1><?php if (!empty($post->name)) { echo $post->name; } else { echo $title; } ?></h1><?php if (!empty($post->content)) { echo '<div class="zdes">' . $post->content . '</div>'; } ?></div>
<div id="list_left_ad1"><?php echo get_ad_code('list_left_ad1'); ?></div>
...
<div class="info"><span class="fl"><em><?php echo date("m-d H:i", $row['update_time']); ?></em></span><span class="fr"><em><?php echo $row['click']; ?></em>人阅读</span></div><div class="cl"></div></div><?php } } else { echo '<div style="text-align:center;padding:20px;">暂无记录</div>'; } ?>
<div id="list_left_ad2"><?php echo get_ad_code('list_left_ad2'); ?></div>

<div class="bootstrap-front-pagination"><?php echo $page; ?><div class="cl"></div></div><div id="lad3"><?php echo get_ad_code('left_ad_w640'); ?></div></div><!-- fl_640 end -->

<div class="fr_300"><div id="rad1"><?php echo get_ad_code('right_ad1'); ?></div>
<div class="side"><div class="stit"><h2>推荐阅读</h2><a href="javascript:getmore({limit:5<?php if (!empty($post->id)) { echo ',type_id:' . $post->id; } ?>,tuijian:1,mode:1,orderby:'rand()'});" class="more">换一换</a><div class="cl"></div></div>	
...
<div id="rad2"><?php echo get_ad_code('right_ad2'); ?></div>
<div class="side"><div class="stit"><h2>猜你喜欢</h2><a href="javascript:getmore({limit:5<?php if (!empty($post->id)) { echo ',type_id:' . $post->id; } ?>,mode:2,orderby:'rand()'});" class="more">换一换</a><div class="cl"></div></div>
...
<div id="rad3"><?php echo get_ad_code('right_ad3'); ?></div></div><!-- fr_300 end --></div><!-- box end -->
<script>
