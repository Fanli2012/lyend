...
<h1 class="arct" style="padding-left:20px;padding-right:20px;"><?php echo $post->title; ?></h1><?php if (false) { ?><div class="timely"><?php if (!empty($post->writer)) { echo '<span id="awriter">' . $post->writer . '</span> '; } ?><span id="atime"><?php echo date("Y-m-d H:i", $post->update_time); ?></span> <?php if (!empty($post->source)) { ?><?php echo '<span id="asource">' . $post->source . '</span> '; ?><?php } ?><span id="aclick">阅读:<?php echo $post->click; ?>次</span> <span id="akeyword">关键词:<?php echo $post->keywords; ?></span></div><?php } ?>
<div class="dad1"><?php echo get_ad_code('article_title_bottom'); ?></div>
<div class="content"><?php echo $post->content; ?><div class="dad2"><?php echo get_ad_code('article_content_bottom'); ?></div>
<div class="dinfo"><span class="fr">下一篇：<?php if ($previous_article) { ?><a href="<?php echo route('index_article_detail', array('id'=>$previous_article->id)); ?>"><?php echo $previous_article->title; ?></a><?php } else { echo '没有了'; } ?></span><div class="cl"></div></div>
</div><div class="dad3"><?php echo get_ad_code('left_ad_w640'); ?></div>
</div><!-- fl_640 end -->
...
<ul class="uli chs" id="xglist"><?php if ($relate_zuixin_list) { foreach ($relate_zuixin_list as $row) { ?><li><a href="<?php echo route('index_article_detail', array('id'=>$row->id)); ?>"><?php echo $row->title; ?></a></li><?php } } ?></ul><div class="cl"></div></div>
